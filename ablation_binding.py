...
for rep in range(replicate):
    ...
    if config['model'] == 'chrombpnet':
        ...
        acc_dna_cnn = acc_dna_cnn = DNA_CNN(n_filters=n_filters,)

        acc_dilated_cnn = DilatedCNN(n_filters=n_filters,
                                     n_layers=n_layers,
                                     kernel_size=3,
                                     activation=activation,
                                     batch_norm=batch_norm,
                                     residual=True)
        acc_head = Bindingscore_head(n_filters,
        ...
        acc_dna_cnn = DNA_CNN_Enformer(n_filters=n_filters,
                                       kernel_size=21,
                                       padding=10,
                                       in_channels=4,
                                       pool_size=pool_size,
        ...
        if n_dilated_layers > 0:
            acc_hidden.append(DilatedCNN(n_filters=n_filters,
                                     ...
                                     residual=True))
        if n_conv_layers > 0:
            acc_hidden.append(ConvTower(in_channels=n_filters,
        ...
        if n_trans_layers > 0:
            ...
            acc_hidden.append(AttentionTower(
                input_dim=768,
                n_head=8,
                d_model=768,
                d_k=64,
        ...
        acc_head = Bindingscore_head(768 if n_trans_layers else n_filters,
                                     kernel_size=[1],
        ...
        print (dna_len, output_len_needed_in_X, x_shape, trim, x_shape_final, output_len_final)
    ...
    n_epoch = acc_model.fit(dataloader['train'],
    ...
    valid_loss, valid_within, valid_across = validation_step_footprint(acc_model,
                                                                       dataloader['valid'],
                                                                       None,
                                                                       mode=[config['loss']], verbose=True)
    valid_loss, valid_within, valid_across = float(valid_loss[0]), float(valid_within[0]), float(valid_across[0])
