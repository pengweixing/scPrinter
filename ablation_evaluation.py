...
for rep in range(1):
    ...
    attribution_to_bigwig(pred_all[:, 0, :].cpu().numpy() / 10 + 0.5 if config['loss'] == 'regression' else torch.sigmoid(pred_all[:, 0, :].cpu()).numpy(),
                          pd.DataFrame(regions),
                          dataloader['test'].dataset.chrom_size,
                          res=1,
                          mode='average', output=savename+'.bigwig')
    ...
    for method in methods:
        print ("method", method)
        attributions = calculate_attributions(ProfileWrapperFootprintClass(acc_model),
                                              model_output=None,
                                              X=datasets['test'].cache_seqs,
                                              n_shuffles=20,
                                              method=method, verbose=True)
        projected_attributions = projected_shap(attributions, datasets['test'].cache_seqs, 64, 'cuda')
        attribution_to_bigwig(projected_attributions,
                              pd.DataFrame(regions),
                              dataloader['test'].dataset.chrom_size,
                              res=1,
                              mode='average', output=savename+'.attr.'+method+'.bigwig')
