...
class RelMultiHeadAttention(nn.Module):
    ...
    def __init__(self, input_dim, n_head, d_model, d_k, d_v, dropout, simple_attention=False):
        super().__init__()
    ...
    def forward(self, q, k=None, v=None):
        ...
        if not self.simple_attention:
            ...
            out = self.w_out(
                flash_attn_func(q.half(), k.half(), v.half(), dropout_p=self.attn_dropout)
                .float()
                .contiguous()
                .view(sz_b, len_q, -1)
            )
...
class DNA_CNN_Enformer(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            nn.Conv1d(in_channels, n_filters, kernel_size, padding=padding),
...
class ConvTower(nn.Module):
    def __init__(
        ...
        kernel_nums = [in_channels] + [int(in_channels * base**i) for i in range(n_layers)]
        ...
        for dim_in, dim_out in zip(kernel_nums[:-1], kernel_nums[1:]):
            conv_layers.append(
                nn.Sequential(
                    ...
                    (
                        AttentionPool(dim_out, pool_size=pool_size)
                        if pool_size > 1
                        else nn.Identity()
                    ),
                )
