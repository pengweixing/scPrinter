...
def calculate_attributions(model, X, method="deeplift", n_shuffles=20, verbose=False, n_steps=10):
    ...
    for cluster in tqdm(order_of_cluster):
        ...
        if len(mask) == 1:
            ...
            bw.addEntries(
                str(chrom),
                points,
                values=attributions_chrom[0].astype("float"),
                span=res,
            )
            continue
