...
class DilatedCNN(nn.Module):
    ...
    def __init__(
        ...
        self.batch_norm_layers = nn.ModuleList(
            [nn.BatchNorm1d(n_filters) if batch_norm else nn.Identity() for i in range(n_layers)]
        )

        self.activation_layers = nn.ModuleList([copy.deepcopy(activation) for i in range(n_layers)])
...
class BPNet_head(nn.Module):
    def __init__(
        ...
        self.linear = nn.Linear(
            in_features=n_filters, out_features=n_output, bias=count_output_bias
        )
...
@torch.no_grad()
def validation_step(model, validation_data, validation_size):
    size = 0
    y_count_all = []
    X_count_all = []
    profile_pearson = []
    ...
    for data in validation_data:
        ...
        val_profile_loss += (
            multinomial_negative_log_likelihood(
                pred_profile.reshape(pred_profile.shape[0], -1),
                y.reshape((y.shape[0], -1)),
            )
            .sum()
            .item()
        )
        profile_pearson.append(batch_pearson_correlation(pred_profile, y))
    ...
    count_pearson = (
        batch_pearson_correlation(X_count_all.T, torch.log1p(y_count_all).T).mean().item()
    )
    ...
    def fit(
        ...
        for epoch in range(max_epochs):
            ...
            for data in training_data:
                ...
                profile_loss = multinomial_negative_log_likelihood(
                    pred_profile, y.reshape((y.shape[0], -1))
                ).mean()
                ...
                bar.update(1)
                iteration += 1
                moving_avg_loss += np.array([loss.item(), profile_loss.item(), count_loss.item()])
            ...
            val_profile_loss, count_mse, profile_pearson, count_pearson = validation_step(
                self, validation_data, validation_size
            )
...
class ChromBPNet(BPNetBase):
    ...
    def __init__(self, bias_model=None, accessibility_model=None, fix_bias_model=True):
        super().__init__()
        if fix_bias_model:
            bias_model.eval()
    ...
    def forward(self, X):
        ...
        counts = torch.logsumexp(torch.stack([count_acc, count_bias]), dim=0)
