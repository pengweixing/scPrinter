...
def bias_model_hyperparam(train_peak_dataset,
                          valid_peak_dataset,
    ...
    print ("Number of nonpeaks after applying upper-bound cut-off and removing outliers : ",
...
def chrombpnet_model_hyperparam(train_peak_dataset,
                            valid_peak_dataset,
                            train_nonpeak_dataset,
                            valid_nonpeak_dataset,
    ...
    valid_nonpeak_cov = valid_nonpeak_dataset.coverage.sum(axis=-1)
    train_valid_nonpeak_cov = np.concatenate([train_nonpeak_cov, valid_nonpeak_cov], axis=0)
    neg_index = np.random.permutation(len(train_valid_nonpeak_cov))[:int(len(train_valid_cov)
                                                                         * negative_sampling_ratio)]
    final_counts = np.concatenate([train_valid_cov,
                                   train_valid_nonpeak_cov[neg_index]], axis=0)
    ...
    print("Number of peaks after removing outliers: ", len(train_peak_dataset) + len(valid_peak_dataset))
    print("Number of nonpeaks after removing outliers: ", len(train_nonpeak_dataset) + len(valid_nonpeak_dataset))

    # somehow it uses <= and >= istead of < and >, consistent with their code
    count_loss_weight = np.median(final_counts[(final_counts >= lower_threshold)
                                  & (final_counts <= upper_threshold)]) / 10
    print ("count_loss_weight", count_loss_weight)
