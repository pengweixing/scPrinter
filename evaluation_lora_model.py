...
mm = acc_model.embeddings.weight.data[:, -1].mean()
# scale = torch.sum(torch.abs(model.embeddings.weight.data)) / torch.sum(torch.abs(torch.from_numpy(grp2embeddings).float()))
# print ("scale", scale)
# model.embeddings.weight.data = torch.from_numpy(grp2embeddings).float() * scale
acc_model.embeddings.weight.data[:, -1] = mm
acc_model.eval()
        ...
        for method in methods:
            ...
            if len(gpus) > 1:
                ...
                for id in ids:
                    ...
                    #                       res=1,
                    #                       mode='average', output=os.path.join(save_dir,f'model_{id}.attr.{wrapper}.{method}{extra}.{decay}.bigwig'))
                    np.savez(
                        os.path.join(
                            save_dir, f"model_{id}.hypo.{wrapper}.{method}{extra}.{decay}.npz"
                        ),
