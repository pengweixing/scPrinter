...
regions = np.array([summits[:, 0], summits[:, 1] - dna_len // 2,
                    summits[:, 1] + dna_len // 2]).T
...
for wrapper in wrappers:
    for n_out in nth_output:
        for method in methods:
            extra = f'_{n_out}_'
            if len(gpus) > 1:
                if write_bigwig and os.path.exists(os.path.join(save_dir, f'attr.{wrapper}.{method}{extra}.{args.decay}.bigwig')) and (not args.overwrite):
                    print ("exists")
                    continue
                ...
                commands = [
                    ['python', 'evaluation_model.py',
                     ...
                     '--method', method, '--write_numpy',
                     '--wrapper', wrapper, '--nth_output', str(n_out), '--gpus', str(gpu),] + (['--overwrite'] if args.overwrite else [])
                ...
                for start, end in zip(starts, ends):
                    attributions.append(np.load(os.path.join(save_dir,f'attr.{wrapper}.{method}{extra}.{start}-{end}.npy')))
                    hypo.append(np.load(os.path.join(save_dir,f'hypo.{wrapper}.{method}{extra}.{start}-{end}.npz'))['arr_0'])
                    ohe.append(np.load(os.path.join(save_dir,f'ohe.{wrapper}.{method}{extra}.{start}-{end}.npz'))['arr_0'])
                ...
                if wrapper == 'classification':
                    model = ProfileWrapperFootprintClass(acc_model, nth_output=n_out, res=1, reduce_mean=False, decay=args.decay)
                elif wrapper == 'regression':
                    model = ProfileWrapperFootprint(acc_model, nth_output=n_out, res=1, reduce_mean=False)
                elif wrapper == 'classification_reduce':
                    model = ProfileWrapperFootprintClass(acc_model, nth_output=n_out, res=1, reduce_mean=True, decay=args.decay)
                elif wrapper == 'regression_reduce':
                    model = ProfileWrapperFootprint(acc_model, nth_output=n_out, res=1, reduce_mean=True)
                ...
                if write_bigwig and os.path.exists(os.path.join(save_dir,f'attr.{wrapper}.{method}{extra}.{args.decay}.bigwig')) and (not args.overwrite):
                    print ("exists")
                    continue
                elif not write_bigwig and os.path.exists(os.path.join(save_dir,f'attr.{wrapper}.{method}{extra}.{start}-{end}.npy')) and (not args.overwrite):
                    print ("exists")
                    continue
                datasets['test'].cache()
                attributions = calculate_attributions(model,
                                                      X=datasets['test'].cache_seqs[start:end],
                                                      n_shuffles=20,
                                                      method=method, verbose=True)
                projected_attributions = projected_shap(attributions,
                                                        datasets['test'].cache_seqs[start:end],
                                                        64, 'cuda')
                hypo, ohe = attributions.detach().cpu().numpy(), datasets['test'].cache_seqs[start:end].detach().cpu().numpy()

                # projected_attributions = np.zeros((end - start, 2114))
            if write_bigwig:
                print (projected_attributions.shape, hypo.shape, ohe.shape, regions)
                np.savez(os.path.join(save_dir, f'hypo.{wrapper}.{method}{extra}.{args.decay}.npz'), hypo)
                ...
                attribution_to_bigwig(projected_attributions,
                                      pd.DataFrame(regions),
                                      dataloader['test'].dataset.chrom_size,
                                      res=1,
                                      mode='average', output=os.path.join(save_dir,f'attr.{wrapper}.{method}{extra}.{args.decay}.bigwig'))

                if len(gpus) > 1:
                    for start, end in zip(starts, ends):
                        os.remove(os.path.join(save_dir,f'attr.{wrapper}.{method}{extra}.{start}-{end}.npy'))
                        os.remove(os.path.join(save_dir,f'hypo.{wrapper}.{method}{extra}.{start}-{end}.npz'))
                        os.remove(os.path.join(save_dir,f'ohe.{wrapper}.{method}{extra}.{start}-{end}.npz'))
            else:
                if len(gpus) <= 1:
                    np.save(os.path.join(save_dir,f'attr.{wrapper}.{method}{extra}.{start}-{end}.npy'), projected_attributions)
                    np.savez(os.path.join(save_dir,f'hypo.{wrapper}.{method}{extra}.{start}-{end}.npz'), hypo)
                    np.savez(os.path.join(save_dir,f'ohe.{wrapper}.{method}{extra}.{start}-{end}.npz'), ohe)
