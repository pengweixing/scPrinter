...
if len(count_feats) > 0 and (not args.avg):
    ...
    count_pred = torch.stack(
        [
            torch.sigmoid(count_model(count_feats)).cpu().detach()[:, 0, :],
            torch.flip(torch.sigmoid(count_model(count_feats_rev)).cpu().detach()[:, 0, :], [1]),
        ],
        axis=0,
    )
    foot_pred = torch.stack(
...
if args.temp:
    attribution_to_bigwig(
        ...
        mode="average",
        output=(
            (save_name + "TFBS_count.bigwig") if not args.avg else (save_name + "avg_count.bigwig")
        ),
        verbose=not silent,
    )
    attribution_to_bigwig(
        ...
        mode="average",
        output=(
            (save_name + "TFBS_foot.bigwig") if not args.avg else (save_name + "avg_foot.bigwig")
        ),
        verbose=not silent,
    )
