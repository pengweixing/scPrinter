...
parser.add_argument('--avg', action='store_true', default=False, help='just avg without finetune model')
...
for i in trange(len(summits), disable=silent):
    ...
    count_feat = np.nanmean([np.nan_to_num(i.values(chrom, summit - signal_window, summit + signal_window, numpy=True))
                             for i in seq_count], axis=0)
    foot_feat = np.nanmean([np.nan_to_num(i.values(chrom, summit - signal_window, summit + signal_window, numpy=True))
                            for i in seq_foot], axis=0)
    ...
    if len(count_feats) == batch_size and (not args.avg):
        ...
        count_pred = torch.stack([torch.sigmoid(count_model(count_feats)).cpu().detach()[:, 0, :],
                      torch.flip(torch.sigmoid(count_model(count_feats_rev)).cpu().detach()[:, 0, :], [1])], axis=0)
        foot_pred = torch.stack([torch.sigmoid(foot_model(foot_feats)).cpu().detach()[:, 0, :],
                     torch.flip(torch.sigmoid(foot_model(foot_feats_rev)).cpu().detach()[:, 0, :], [1])], axis=0)
...
attribution_to_bigwig(avg,
                      ...
                      verbose=not silent)
if args.temp:
    attribution_to_bigwig(count_pred,
                          regions,
                          genome.chrom_sizes,
                          res=1,
                          mode='average',
                          output=(save_name + 'TFBS_count.bigwig') if not args.avg else (save_name + "avg_count.bigwig"),
                          verbose=not silent
                          )
    attribution_to_bigwig(seq_pred,
                          regions,
                          genome.chrom_sizes,
                          res=1,
                          mode='average',
                          output=(save_name + 'TFBS_foot.bigwig') if not args.avg else (save_name + "avg_foot.bigwig"),
                          verbose=not silent)
