...
def _get_region_atac(printer, cell_barcodes, region):
...
def get_region_atac(printer, cell_barcodes, region):
...
def _get_group_atac(printer, cell_grouping_idx, region):
    # why slice(None)?
...
# get ATAC insertion for groups and a given region through bigwig
def _get_group_atac_bw(dict1, cell_grouping, region):
...
# A wrapper function for more complicated situations
def get_group_atac(printer, cell_grouping, region):
    region = regionparser(region, printer)
