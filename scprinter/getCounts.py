...
def fragmentsToInsertion(frags, # Chunks of fragments
    ...
    insertion_left.columns = insertion_right.columns = ['Chromosome', 'Start', 'Barcode']
    ...
    insertion = insertion.intersect(regions,
                                    wa=True,
                                    wb=True).to_dataframe(names=['Insertion_Chrom', 'Insertion_Start',
                                                                 'Insertion_End', 'Barcode'] + region_columns)
    ...
    summary = insertion.groupby(by=['position', 'Group', 'region_id']).sum().reset_index().sort_values(
        by=['Group', 'position'])
...
def mergeRegionCountTensor(count1, count2):
    ...
    df = pd.DataFrame(np.concatenate([count1, count2], axis=0), columns=['position', 'Group', 'count'])
    summary = df.groupby(by=['position', 'Group']).sum().reset_index().sort_values(
        by=['Group', 'position'])
...
def computeCountTensor(pathToFrags,  # Path or list of paths to fragments file
    ...
    uniq_group, existing_groups, new_group, \
        uniq_region, existing_region, new_region = Unify_meta_info(final_hdf5,
                                                addition_feats = [np.unique(barcodeGroups['group']),
                                               region_identifier],
    ...
    if append_mode == 1:
        ...
        assert len(new_group) == 0, "append_region mode, but observed changed cell grouping: %s ..." %new_group[0]
        assert len(new_region) == len(uniq_region), "append mode, cannot have overlap regions: %s"  % uniq_region[np.isin(uniq_region, existing_region)][0]
    elif append_mode == 2:
        # append group mode, check if there are new regions
        assert len(new_region) == 0, "append_group mode, but observed changed regions: %s..."  %new_region[0]
        assert len(new_group) == len(uniq_group), "append mode, cannot have overlap groups: %s" % uniq_group[np.isin(uniq_group, existing_groups)][0]
    elif append_mode == 3:
        assert len(new_group) == 0 and len(new_region) == 0, "append_frags mode, but region / " \
    ...
    for pathToFrags in pathsToFrags:
        ...
        for chunk_fragments_count, chunk_fragments in enumerate(reader):
            ...
            p_list.append(pool.submit(fragmentsToInsertion, chunk_fragments, maxFragLength,
                                      regions, region_columns, extra_plus_shift,
                                      extra_minus_shift, barcodeGroups_dict, chunkSize))
            submit_total += 1
            print("submitting %d jobs, takes: %.2f s\r" % (chunk_fragments_count, time.time() - start), end="")
                ...
                while len(p_list) > nCores:
                    for p in as_completed(p_list):
                        ...
                        for chunk in insertions:
                            ...
                            hdf5_storage.put("chunk_%d" % (chunk), xx,
                                             format='table', append=True, index=False)
                        p_list.remove(p)
                        del p
        ...
        for p in as_completed(p_list):
            ...
            insertions = p.result()
            for chunk in insertions:
                hdf5_storage.put("chunk_%d" % (chunk), insertions[chunk],
                                 format='table', append=True, index=False)
    ...
    if returnCombined:
        with h5py.File(final_hdf5_path, 'r') as f:
            return_list = [np.array(f[region_identifier[i]]) if region_identifier[i] in f else None for i in range(len(regions))]
            return return_list


def appendCountTensor_regions(*args, # dataframe or list of dataframe specifying membership of barcodes in pseudobulks.
                       **kwargs):
    computeCountTensor(*args, append_mode=1, **kwargs)


def appendCountTensor_groups(*args, # dataframe or list of dataframe specifying membership of barcodes in pseudobulks.
                       **kwargs):
