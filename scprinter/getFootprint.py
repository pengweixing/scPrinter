...
def getRegionATAC(
    select,  # np array of (, 3): position, group, count
...
def footprintScoring_multi(
    ...
    if return_pval:
        ...
        leftPval = scipy.stats.norm.cdf(
            fgLeftRatio, leftPredRatioMean, leftPredRatioSD
        )  # / scipy.stats.norm.cdf(fgLeftRatioBias, leftPredRatioMean2, leftPredRatioSD2)

        # This is to make it consistent with R pnorm
        leftPval[np.isnan(leftPval)] = 1
        rightPval = scipy.stats.norm.cdf(
            fgRightRatio, rightPredRatioMean, rightPredRatioSD
        )  # / scipy.stats.norm.cdf(fgRightRatioBias, rightPredRatioMean2, rightPredRatioSD2)
...
# torch jit version
@torch.jit.script
def predictDispersion_jit(x, modelWeights_0, modelWeights_1, modelWeights_2, modelWeights_3):
        ...
        for i, region_identifier in enumerate(region_identifier_chunk):
            ...
            if return_array is None:
                return_array = np.zeros(
                    (len(region_identifier_chunk), result.shape[0], result.shape[-1])
                )
...
def getFootprints(
    ...
    with h5py.File(CountTensorPath, "r") as f:
        ...
        (
            uniq_group,
            existing_counttensor_groups,
            group_notin_counttensor,
            uniq_region,
            existing_region,
            region_notin_counttensor,
        ) = Unify_meta_info(
            f,
        ...
        assert len(group_notin_counttensor) == 0, (
            "observe new groups that don't have count tensor: %s..." % group_notin_counttensor[0]
        )
        assert len(region_notin_counttensor) == 0, (
            "cannot have new regions that don't have count tensor: %s..."
    ...
    if saveDir is not None:
        ...
        if append_mode == 1:
            ...
            assert len(new_scale) == 0, (
                "append_region mode, but observed changed scales: %d ..." % new_scale[0]
            )
            ...
            assert len(new_scale) == 0, (
                "append_group mode, but observed changed scales: %d ..." % new_scale[0]
            )
            ...
            assert len(new_group) == 0, (
                "append_scale mode, but observed changed cell grouping: %s ..." % new_group[0]
            )
            assert len(new_region) == 0, (
                "append_scale mode, but observed changed regions: %s..." % new_region[0]
            )
            assert len(new_scale) == len(uniq_scale), (
                "append mode, cannot have overlap scale: %d"
                % uniq_scale[np.isin(uniq_scale, existing_scale)][0]
            )
    ...
    for i in range(0, len(regions), small_chunk_size):
        ...
        for mode, r1, r2 in zip(modes, footprintRadius, flankRadius):
            p_list.append(
                pool.submit(
                    ...
                    int(
                        mode
                    ),  # int or list of int. This is used for retrieving the correct dispersion model.
                    r1,
                    r2,
                )
            )
        ...
        if returnCombined:
            if final_return_array is None:
                final_return_array = np.zeros([len(regions)] + list(return_array.shape[1:]))
...
def appendFootprints_regions(*args, **kwargs):
    getFootprints(*args, append_mode=1, **kwargs)


def appendFootprints_groups(*args, **kwargs):
