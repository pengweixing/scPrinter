...
class Footprinter:
    def __init__(
        ...
        self.dispersionModels = loadDispModel(
            os.path.join(model_dir, "shared", "dispModel", "models.h5")
        )
        self.TFBSModel = loadBindingScoreModel(
            os.path.join(model_dir, "TFBSPrediction", "TFBS_model_py.h5")
        )
        self.BindingScoreModel = {"TF": self.TFBSModel}
        self.precomputed_bias_path = os.path.join(
            model_dir, "shared", "%sTn5Bias.h5" % self.refGenome
        )
        self.Tn5Bias = getPrecomputedBias(self.precomputed_bias_path, self.regions, savePath=None)
