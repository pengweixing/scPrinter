...
def prep_peak(file,
    ...
    if len(peaksR) > len(np.unique(peaksR['name'])):
        ...
        duplicates = peaksR.duplicated(subset='name',
                                   keep='first')  # Keep the first occurrence of each duplicated value
        ...
        for index, is_duplicate in enumerate(duplicates):
            if is_duplicate:
                ...
                try:
                    names[index] = names[index] + strings[mapping[names[index]] - 1: mapping[names[index]]+1]  # Mapping to a, b, c, ...
                except:
    ...
    if use_quantile_scores:
        peaksR['final_score'] = np.round(quantile_transform(np.array(peaksR['score'])[:, None],
                                             n_quantiles=min(1000, len(peaksR)), output_distribution='uniform', copy=True).squeeze(), 3)
    else:
...
def make_peaks_df_bioframe(summit_files,
                           names,
                           peak_width,
                           genome,filter_chr=False,
                           blacklist_peak_width=None,
                           n=999999999, fdr_threshold=0.01,
                           use_quantile_scores=False,
                           final_peak_width=None, max_iter=1000,
                           ):
    ...
    print("Removing peaks overlapping with blacklisted regions and out of bound peaks based on chromosome sizes ..\n")
    ...
    func = partial(prep_peak, chrs=chrs, fdr_threshold=fdr_threshold, use_quantile_scores=use_quantile_scores,
                     pad=blacklist_pad, blacklist=blacklist, chrom_sizes=chrom_sizes)

    peaks = list(process_map(func, zip(summit_files, names),
    ...
    while len(peakscoverage) > 0:
        i += 1
        peak_select = cluster_remove_peaks_bioframe(peakscoverage, by="final_score", decreasing=True)
        print("round:", i, len(peakscoverage), "peaks unresolved", len(peak_select), "peaks selected")
    ...
    cleaned_peaks = fP.sort_values(by="final_score", ascending=False).head(nout).sort_values(by=["chrom", "start", "end"])
    print ("finish sorting")
    summary = bioframe.overlap(cleaned_peaks, raw_peaks).groupby('name').agg({
        'chrom': 'first',
        'start': 'first',
        'end': 'first',
        'name_': list,
...
def clean_macs2(name, outdir,
                peak_width,
                genome, filter_chr=False,
                blacklist_peak_width=None,
                n=999999999, fdr_threshold=0.01, use_quantile_scores=False, final_peak_width=None,
                max_iter=1000,
                ):
    cleaned_peaks, _ = make_peaks_df_bioframe([os.path.join(outdir, name + '_summits.bed')],
                                                  [name],
                                                  peak_width,
                                                  genome, filter_chr=filter_chr,
                                                    blacklist_peak_width=blacklist_peak_width,
                                                  n=n, fdr_threshold=fdr_threshold, use_quantile_scores=use_quantile_scores,
                                                  final_peak_width=final_peak_width, max_iter=max_iter)
    return cleaned_peaks

def macs2(frag_file, name, outdir, format='BEDPE'):
    commands = ["macs2", "callpeak", "--nomodel", "-t",
                frag_file,
                "--outdir", outdir,
                "-n", name,
                "-f", format,
