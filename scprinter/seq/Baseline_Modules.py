...
class BiasInjection(nn.Module):
    def __init__(self, layer,
        ...
        self.embedding_nn = nn.Sequential(
            ...
            out_features=self.layer_dim_in
            ),
        )
    ...
    def forward(self, X, cells, modes=None):
        ...
        if modes is None:
            ...
            X = X + bias[..., None]
            return F.conv1d(X, self.layer.weight[modes],
                                            self.layer.bias[modes], padding=self.layer.padding[0])
    ...
    def collapse_layer(self, cell):
        ...
        self.bias = self.embedding_nn(torch.tensor([cell]).long().to(self.embedding_nn[0].weight.data.device))
