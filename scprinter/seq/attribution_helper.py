...
def multiscale_attribution(model,
                           X,
    ...
    if method != 'ism':
        _references = torch.cat([dinucleotide_shuffle(xx, n_shuffles=n_shuffles).to(dev) for xx in X], dim=0)
        ...
        for output in tqdm(outputs, disable=not verbose):
            ...
            attributions = calculate_attributions(model, X,  method, n_shuffles, references=_references)
...
def multiscale_ism(model,
    ...
    seqs = [printer.genome.fetch_onehot_seq(region[0],
                                            region[1],
                                            region[2]) for region in np.array(regions)]
    attrs = [_multiscale_ism(model,
                             X_0,
                             scales=scales,
                             ...
                             project=project,
                             sum=sum,
                             batch_size=batch_size) for X_0 in seqs]
    return attrs

def summary_func(logits, nth_output,
                 specific_pos=None,
                 reduce_mean=True):
...
@torch.no_grad()
def _mutation_footprints(model,
                         X_0,
                         batch_size=64,
                         verbose=False):
    dev = model.parameters().__next__().device
...
def _multiscale_ism(model,
    ...
    reference = model(X_0[None].float().to(dev))[0] # 0 for the shape
    def normalize(x, ):
        # x = F.softmax(x, dim=-1) * (x - x.mean(dim=-1, keepdims=True))
        shapes = x.shape
    ...
    # references = [summary_func(reference, scale,
    #                            specific_pos=None,
    #                            reduce_mean=attr_reduce_mean) for scale in scales]
    ...
    for start in tqdm(starts, disable=not verbose):
        ...
        if effect_range is None:
            for i, scale in enumerate(scales):
                # ism[i].append(summary_func(y, scale,
                #                       specific_pos=None,
                #                       reduce_mean=attr_reduce_mean) - summary_func(reference, scale,
                #                         specific_pos=None,
                #                         reduce_mean=attr_reduce_mean))
                # ism[i].append(diffs[:, scale].sum(dim=1).sum(dim=1))
                ism[i].append((normalize(y[:, scale]) - normalize(reference[:, scale])).sum(dim=(1, 2)))
        else:
            changed_position = torch.arange(start, start + X_.shape[0]) // (n_choices - 1)
            offset = (seq_len - reference.shape[-1]) // 2 #(offset because, the original sequence is padded)
            ...
            for i, scale in enumerate(scales):
                tmp = []
                diff = (normalize(y[:, scale]) -
                                normalize(reference[:, scale]))
    ...
    for scale in range(len(scales)):
        isms = ism[scale].reshape(seq_len, n_choices - 1) # reshaped to seq_len, n_choices - 1

        j_idxs = torch.arange(seq_len)
        X_ism = torch.zeros(seq_len, n_choices, device='cuda')
        ...
        X_ism = (X_ism - X_ism.mean(dim=0, keepdims=True)) # X_ism calculates deltas, as in the difference between the original and the perturbed
...
def multiscale_visualization(attr,
                             cropp_len=800,
                             ax=None,
                             rainbow=False,
                             cbar=False):
    ...
    if rainbow:
        ...
        sns.heatmap(attr[0][::-1].sum(axis=1), cmap='Blues', vmin=vmin, vmax=vmax, ax=ax, cbar=cbar)
