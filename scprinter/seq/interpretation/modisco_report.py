...
def fetch_tomtom_matches(
    ...
    if not shutil.which(tomtom_exec_path):
        raise ValueError(
            f"`tomtom` executable could not be called globally or locally. Please install it and try again. You may install it using conda with `conda install -c bioconda meme`"
        )

    # run tomtom
    cmd = (
        "%s -no-ssc -oc . --verbosity 1 -text -min-overlap 5 -mi 1 -dist pearson -evalue -thresh 10.0 %s %s > %s"
...
def create_modisco_logos(
    modisco_h5py: os.PathLike, modisco_logo_dir, trim_threshold, pattern_groups: List[str]
):
...
def report_motifs(
    ...
    is_writing_tomtom_matrix: bool,
    top_n_matches=3,
    delta_effect_path: os.PathLike = None,
    delta_effect_prefix="",
    trim_threshold=0.3,
    trim_min_length=3,
):
    ...
    with h5py.File(modisco_h5py, "r") as modisco_results:
        for name in pattern_groups:
            ...
            for pattern_name, pattern in sorted(metacluster.items(), key=key):
                ...
                pattern_tag = f"{delta_effect_prefix}_{name}.{pattern_name}"
            ...
            for _, row in patterns_df.iterrows():
                if name in patterns_df.columns:
                    if pandas.isnull(row[name]):
                        ...
                        make_logo(row[name], img_path_suffix, motifs)
