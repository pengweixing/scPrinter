...
class PyTorchGradient(Explainer):
    ...
    def gradient(self, idx, inputs):
        ...
        if self.input_handle is not None:
            ...
            grads = [
                torch.autograd.grad(selected, x, retain_graph=True if idx + 1 < len(X) else None)[0]
                .cpu()
                .numpy()
    ...
    def shap_values(
        ...
        samples_input = [
            torch.zeros((nsamples,) + X[t].shape[1:], device=X[t].device) for t in range(len(X))
        ]
        samples_delta = [
            np.zeros((nsamples,) + self.data[t].shape[1:]) for t in range(len(self.data))
        ]
                ...
                for k in range(nsamples):
                    ...
                    for a in range(len(X)):
                        ...
                        if self.input_handle is None:
                            samples_delta[a][k] = (
                                (x - (self.data[a][rind]).clone().detach()).cpu().numpy()
                            )

                    if self.interim is True:
                        with torch.no_grad():
                            _ = self.model(
                                *[samples_input[a][k].unsqueeze(0) for a in range(len(X))]
                            )
                            interim_inputs = self.layer.target_input
                            del self.layer.target_input
                            if type(interim_inputs) is tuple:
