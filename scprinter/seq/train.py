...
from ..utils import load_entire_hdf5, loadDispModel
...
def train_model(model_configs, device="cuda", wandb_info=None):
    ...
    acc_head = Footprints_head(
        n_filters, kernel_size=head_kernel_size, n_scales=len(modes), per_peak_feats=1
    )
    output_len = 800
    ...
    n_epoch, loss_history = acc_model.fit(
        dispmodel,
        dataloader["train"],
    ...
    valid_loss, valid_within, valid_across = validation_step_footprint(
        acc_model, dataloader["valid"], None, dispmodel, modes, verbose=True
    )
    ...
    test_loss, test_within, test_across = validation_step_footprint(
        acc_model, dataloader["test"], None, dispmodel, modes, verbose=True
    )
