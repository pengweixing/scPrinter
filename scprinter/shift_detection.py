...
def get_bias(beds, genome, context_radius=20):
    ...
    for frags in np.array(beds):
        ...
        forward_context = one_hot_encode(genome_seq[chrom][start - context_radius:start + context_radius].seq.upper())
        reverse_context = one_hot_encode(genome_seq[chrom][end - context_radius:end + context_radius].seq.upper())
...
def plot_bias(bias_pfm):
    ...
    # Visulize results
    logomaker.Logo(icm, vpad=.1, width=.8)
...
def circular_detect(ref_bias, query_bias):
    ...
    shift_mse = [mse(circular_shift(ref_bias, shift), query_bias) for shift in range(-context_radius, context_radius)]
...
def detect_shift(frags, genome):
    ref_forward_bias, ref_reverse_bias = np.array([[0.22801 , 0.267627, 0.270118, 0.234245],
        ...
        [0.23085 , 0.260809, 0.266385, 0.241956]]), \
                                          np.array([[0.227808, 0.264997, 0.272057, 0.235138],
    ...
    return 4 - circular_detect(ref_forward_bias, forward_bias), -5 - circular_detect(ref_reverse_bias, reverse_bias)