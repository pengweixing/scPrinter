...
def _create_motif_match_trace(motif, chrom, s, e):
    ...
    for tf in tfs[::-1]:
        try:
            tf_df = df.loc[tf]
            base, x, y = (
                np.array(tf_df["Start"]),
                np.array(tf_df["delta"]),
                np.array(tf_df["Level"]),
            )
...
def fetch_importance_values(printer, importance, chrom, start, end):
    if type(importance) is pyBigWig.pyBigWig:
        ...
        with warnings.catch_warnings():
            ...
            v = (
                importance.attribute(seq, additional_forward_args=end - start)
                .detach()
                .cpu()
                .numpy()
            )
...
def sync_footprints(
    ...
    assert (
        "group_bigwig" in printer.insertion_file.uns
    ), "no group_bigwig detected, call pp.sync_footprints first"
    ...
    if gffdb is not None:
        fig.append_trace(
            _create_gene_match_trace(gffdb, chrom, init_s, init_e, max_level + 1),
            row=1,
            col=1,
        )
    ...
    for i, name in enumerate(group_names):
        if name in group_seq_importance:
            importance = fetch_importance_values(
                printer, group_seq_importance[name], chrom, init_s, init_e
            )
            ...
            (all_shapes_, min_neg_height, max_pos_height, height_padding) = plotly_plot_weights(
                seq_imp, xref=xref, yref=yref, start_pos=init_s
            )
            row = seq_axes[name]
            row = 1 if row == "" else int(row)
            # Construct the final Plotly figure with all_shapes
            fig.append_trace(go.Bar(x=[], y=[]), row=row, col=1)
            fig.layout["xaxis" + seq_axes[name]]["range"] = [init_s, init_e]
            fig.layout["yaxis" + seq_axes[name]]["range"] = [
                min_neg_height - height_padding,
        ...
        v = _bigwig_footprint(group_bigwig[name], group_bigwig["bias"], chrom, init_s, init_e, 100)
    ...
    def change_data(self, xrange):
        xrange_zoom_min, xrange_zoom_max = (
        ...
        with fig.batch_update():
            ...
            if gffdb is not None:
                ...
                try:
                    ...
                    base, x, y, gene, strand = [], [], [], [], []
                gene = [xx + "(" + yy + ")" for xx, yy in zip(gene, strand)]
                update_track[trace + 1] = {"x": x, "y": y, "base": base, "text": gene}
            ...
            for i, name in enumerate(group_names):
                if name in group_seq_importance:
                    importance = fetch_importance_values(
                        printer, group_seq_importance[name], chrom, s, e
                    )
                    ...
                    xref = "x" + seq_axes[name]
                    yref = "y" + seq_axes[name]
                    (all_shapes_, min_neg_height, max_pos_height, height_padding) = (
                        plotly_plot_weights(seq_imp, xref=xref, yref=yref, start_pos=s)
                    )
                    fig.layout["yaxis" + seq_axes[name]]["range"] = [
                        min_neg_height - height_padding,
                        max_pos_height + height_padding,
                ...
                v = _bigwig_footprint(group_bigwig[name], group_bigwig["bias"], chrom, s, e, 100)[
                    :, ::step
                ]

                update_track[footprint_data_start + i + offset] = {"z": v, "x": x}
...
def sync_footprints_v2(
    ...
    for i, (type_, name, attr) in enumerate(contents):
        ...
        if type_ == "gene":
            fig.append_trace(
                _create_gene_match_trace(gffdb, chrom, init_s, init_e), row=i + 1, col=1
            )
            ...
            motif_scanner = attr
            figure1_traces, max_level = _create_motif_match_trace(
                motif_scanner, chrom, init_s, init_e
            )
            for traces in figure1_traces:
            ...
            # Construct the final Plotly figure with all_shapes
            fig.append_trace(go.Bar(x=[], y=[]), row=i + 1, col=1)
            fig.layout["xaxis%s" % postfix]["range"] = [init_s, init_e]
            fig.layout["yaxis%s" % postfix]["range"] = [
                min_neg_height - height_padding,
                max_pos_height + height_padding,
            ]
            fig.layout["xaxis%s" % postfix]["visible"] = False
            ...
            v = _bigwig_footprint(
                group_bigwig[attr], group_bigwig["bias"], chrom, init_s, init_e, 100
            )
            x = np.arange(init_s, init_e)
            heatmap = go.Heatmap(
                z=v,
            ...
            mask = ~np.isnan(v)
            fig.append_trace(
                go.Bar(x=x[mask], y=v[mask], showlegend=False, width=1, marker_line_width=0),
                row=i + 1,
                col=1,
            )
            fig.layout["xaxis%s" % postfix]["range"] = [init_s, init_e]
            fig.layout["yaxis%s" % postfix]["range"] = [
                np.quantile(v[mask], 0.01),
                np.quantile(v[mask], 0.99),
            ]
            fig.layout["xaxis%s" % postfix]["visible"] = False
            if not encounter_first_foot:
                first_foot += 1
            ...
            with torch.no_grad():
                ...
                x = np.arange(init_s, init_e)
                fig.append_trace(
                    go.Bar(x=x, y=v, showlegend=False, width=1, marker_line_width=0),
                    row=i + 1,
                    col=1,
                )
                fig.layout["xaxis%s" % postfix]["range"] = [init_s, init_e]
            ...
            v = _bigwig_bindingscore(
                group_bigwig[group_name],
                group_bigwig["bias"],
                model_key,
                chrom,
                init_s,
                ...
                if type_ == "gene":
                    ...
                    for trace, tf in enumerate(tfs[::-1]):
                        try:
                            ...
                            base, x, y = [], [], []
                        update_track[trace + traces_start[i]] = {
                            "x": x,
                            "y": y,
                            "base": base,
                        }
                    ...
                    fig.layout["yaxis" + postfix]["range"] = [
                        min_neg_height - height_padding,
                        max_pos_height + height_padding,
                    ]
                    all_shapes += all_shapes_
                    ...
                    x = np.arange(s, e, step)
                    v = _bigwig_footprint(
                        group_bigwig[attr], group_bigwig["bias"], chrom, s, e, 100
                    )[:, ::step]
