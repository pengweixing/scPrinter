...
def main(config=None, enable_wandb=True):
    ...
    with ((wandb.init(config=config))):
        ...
        grp2barcodes = np.array(pickle.load(open(grp2barcodes, 'rb')))
        ...
        print ("before adjust output layer", val_loss, profile_pearson, across_pearson)
        ...
        acc_model.fit(dispmodel,
                      ...
                      early_stopping=3 if 'early_stopping' not in config else config[
                          'early_stopping'],
                      return_best=True,
